...
func Test_Blocker_Label_BreakFor(t *testing.T) {
	...
	diffString(t, got, lines(
		...
		`    }`,
		`  }`,
		`  block 3 <For-loop Post> {`,
		`    ++j`,
		`    goto(block 2)`,
		`  }`,
		`  block 4 <After For-loop> {`,
		`    return i`,
		`  }`,
		`}`))
}
...
func Test_Blocker_Label_NestedFor(t *testing.T) {
	...
	diffString(t, got, lines(
		...
		`    goto(block 1)`,
		`  }`,
		`  block 1 <For-loop Init for OuterLoop> {`,
		`    i:=0`,
		`    goto(block 2)`,
		`  }`,
		`  block 2 <For-loop Body for OuterLoop> {`,
		`    if !(i < 10) {`,
		`      goto(block 3)`,
		`    }`,
		`    goto(block 4)`,
		`  }`,
		`  block 3 <After For-loop for OuterLoop> {`,
		`    return k`,
		`  }`,
		`  block 4 <For-loop Init for InnerLoop> {`,
		`    j:=3`,
		`    goto(block 5)`,
		`  }`,
		`  block 5 <For-loop Body for InnerLoop> {`,
		`    if !(j >= 0) {`,
		`      goto(block 6)`,
		`    }`,
		`    ++k`,
		...
		`    goto(block 2)`,
		`  }`,
		`  block 6 <After For-loop for InnerLoop> {`,
		`    ++i`,
		`    goto(block 2)`,
		`  }`,
		`}`))
