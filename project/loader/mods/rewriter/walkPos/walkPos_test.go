...
func Test_WalkPos_MultilineFunc(t *testing.T) {
	...
	checkWalkPos(t, fSet, f)(
		...
		`67:ArrayType.Rbrack:1"]"(P)`,
		`68:Ident.Name:3"int"`,
		`71:FieldList.Comma:1","(P)`,
		`73:File.Comment:12"// comment 4"`,
		`87:Ident.Name:1"b"`,
		`89:X.Ellipsis:3"..."`,
		`92:Ident.Name:6"string"`,
		`98:FieldList.Comma:1","(P)`,
		...
		`240:File.End:0`)
	checkWalkPos(t, fSet, f, SkipFileComments, SkipPseudoPos)(
		`1:File.Start:0`,
		`1:File.Package:7"package"`,
		`9:Ident.Name:3"foo"`,
		`27:FuncDecl.Doc:12"// comment 2"`,
...
// TODO: Check if there is a comment before a pseudo, `a int /* Cat */,`

func loadTest(t testing.TB, code ...string) (*token.FileSet, *ast.File) {
	t.Helper()
	...
	f, err := parser.ParseFile(fSet, `test.go`, strings.Join(code, "\n"), mode)
	if err != nil {
...
func checkWalkPos(t testing.TB, fSet *token.FileSet, f *ast.File, options ...WalkPosOption) func(expLines ...string) {
	t.Helper()
	lines := []string{}
	var prior int
	fs := fSet.File(f.FileStart)
	for pt := range WalkPos(fs, f, options...) {
		if !pt.Pos.IsValid() {
