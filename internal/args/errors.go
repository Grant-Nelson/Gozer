...
var (
	...
	ErrToolTagWrongType   = ErrInvalidInput.with(`tool tag is only valid on struct or pointer to struct fields`)
	ErrToolTagRequired    = ErrInvalidInput.with(`tool tag cannot be required`)
	ErrInvalidToolName    = ErrInvalidInput.with(`tool name is not valid`)
	ErrToolAlreadyExists  = ErrInvalidInput.with(`tool name is already defined`)
